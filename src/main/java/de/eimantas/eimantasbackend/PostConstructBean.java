...
public class PostConstructBean implements ApplicationRunner {
  ...
  private void preFillData() {
    ...
    logger.info("Populating Expense From Booking");
    Expense exp = new Expense();
    exp.setAccountId(1);
    exp.setUserId(PopulateStuff.TEST_USER_ID);
    exp.setName("Generated");
    exp.setRefBookingId(11);
    exp.setExpensable(true);
    exp.setExpensed(false);
    exp.setBetrag(BigDecimal.TEN);
    exp.setCategory(ExpenseCategory.GENERATED_NOT_CALCULATE);
    exp.setOrt("Bingen");
    exp.setCreateDate(LocalDateTime.now().toInstant(ZoneOffset.UTC));
    exp.setValid(true);
    exp.setPurpose("Generated by the system for the booking");
    Expense saved = repository.save(exp);
    logger.info("expense saved: " + saved.toString());
